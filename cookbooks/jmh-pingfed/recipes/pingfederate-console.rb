...
include_recipe 'pingfederate::console_instance'

pingfederate_console_dir = File.join(node['pingfed']['install_dir'], 'pingfederate-console', node['pingfed']['filename'],'pingfederate')

# Include JMH specific pingfed config setup
cookbook_file File.join(pingfederate_console_dir, '/server/default/conf', 'pingfederate.lic') do
  source "pingfederate/#{node['jmh_pingfed']['pingfederate']['license_file']}"
  owner node['pingfed']['user']
  group node['pingfed']['user']
  mode 0600
  action :create
  notifies :restart, 'service[pingfederate-console]', :delayed
end

template File.join(pingfederate_console_dir, '/etc/jetty-runtime.xml') do
...
iptables_rule 'pingfederate-console' do
  cookbook 'jmh-pingfed'
  source 'iptables.erb'
...
bash 'extract_pingfed_pcv' do
  cwd ::File.join(pingfederate_console_dir, 'sdk/plugin-src/')
  code <<-EOH
    tar xf /home/pingfederate/src/pingfed-pcv.tar
    chown -R pingfederate. pf-pcv-rest
    mv #{pingfederate_console_dir}/sdk/plugin-src/pf-pcv-rest/src #{pingfederate_console_dir}/sdk/plugin-src/pf-pcv-rest/java
  EOH
  not_if { ::File.exist?("#{pingfederate_console_dir}/sdk/plugin-src/pf-pcv-rest") }
end

jmh_pingfed_install_plugins 'Install pingfederate-console plugins' do
  plugin_array node['jmh_pingfed']['pingfederate']['plugins']
  install_path pingfederate_console_dir
...
template File.join(pingfederate_console_dir, '/server/default/conf/language-packs/pingfederate-messages_en.properties') do
